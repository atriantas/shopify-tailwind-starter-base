(function FAQ(SectionsDesign) {
  ...
  var init = function () {
    var configNodes = Array.from(
      document.querySelectorAll('[data-faq-config]'),
    );
  ...
  function animate(element, direction) {
    ...
    element.animate(
      [{ height: 0 }, { height: element.offsetHeight + 'px' }],
  ...
  function zipObj(keys, values) {
    return keys.reduce(function zipObj(acc, key, idx) {
      acc[key] = values[idx];
      return acc;
    }, {});
  }
